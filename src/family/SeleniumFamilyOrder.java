...
public class SeleniumFamilyOrder {
...
//	private static String picPath = "/home/sixson/eclipse-workspace/family.png";
	...
	private static String picPath = "D:\\javaLib\\selenium-java-3.10.0\\Firefox\\familyorder.png";
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		// 切訂單選項
		wait.until(ExpectedConditions.presenceOfElementLocated(By.id("rdoTX")));
		WebElement changeOrder = driver.findElement(By.id("rdoTX"));
		new Actions(driver).click(changeOrder).perform();
		...
		WebElement datemin, datemax;
		datemin = driver.findElement(By.id("txtTX_DATES"));
		JavascriptExecutor javascriptExecutor06 = (JavascriptExecutor) driver;
		javascriptExecutor06.executeScript("arguments[0].value='" + minDay + "';", datemin);

		String maxDay = day.format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));
		datemax = driver.findElement(By.id("txtTX_DateE"));
		JavascriptExecutor javascriptExecutor07 = (JavascriptExecutor) driver;
		javascriptExecutor07.executeScript("arguments[0].value='" + maxDay + "';", datemax);
...
//		 --------------------------------------------------------訂單處理
		...
		int orderPageTotal = getPageTotalLabel(driver, wait);
		int orderTableTotal;
		System.out.println("共計" + orderPageTotal + "頁");
		for (int j = 1; j <= orderPageTotal; j++) {
...
//					obj = null;
					try {
						...
						liftLink = driver.findElement(By.linkText(getTable.getText().toString()));
						new Actions(driver).click(liftLink).perform();
		...
		System.out.println("訂單:" + requestOrderArray.length() + "筆");
		driver.close();

		// 轉excel表

		JsonArrayToExcel exethis = new JsonArrayToExcel("Farmily訂單", "Farmily");
		exethis.doExcelFromFamily(requestOrderArray);
	...
	private static void getSecondPageData(WebDriver driver, WebDriverWait wait) throws InterruptedException {
		...
		getTable = driver.findElement(
				By.xpath("/html/body/form/table/tbody/tr[2]/td/table/tbody/tr/td/table/tbody/tr[25]/td[4]"));
